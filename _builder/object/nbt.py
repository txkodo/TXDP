...
from builder.base.env import ProvideNbt, Run
...
class NbtBase(Generic[P]):
    ...
    def _match_nbt(self, value: NbtSourceProtocol[T] | P) -> NbtArgument:
    ...
    def Equal(self, value: NbtSourceProtocol[T] | P):
        return NbtCondition(True, self._match_nbt(value))

    def Different(self, value: NbtSourceProtocol[T] | P):
        return NbtCondition(False, self._match_nbt(value))
    ...
    def Exists(self):
        ...
        Run(
            ExecuteConditionCommand(
                [StoreSubCommand("success", result._store_target())],
                ConditionSubCommand("if", NbtConditionArgument(self.nbt)),
            )
        )
        return NbtCondition(True, result._match_nbt(1))
...
class NbtCondition(Condition):
    ...
    def _condition(self) -> ConditionArgument:
        return NbtConditionArgument(self.nbt)
